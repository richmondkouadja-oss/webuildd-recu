...
const LOGO_BASE64 = 'data:image/png;base64,';
...
interface PaymentPDFProps {
  ...
  receipt: {
    ...
    quantity: number;
    unit_price: number;
    receipt_lots?: { ilot_number: string; lot_number: string }[];
  };
  client: {
    full_name: string;
  ...
  paymentIndex: number;  // numéro du versement (1, 2, 3…)
  totalPayments: number; // total des versements sur ce reçu
}
...
export default function PaymentPDF({
  payment, receipt, client, paymentIndex, totalPayments,
}: PaymentPDFProps) {
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <Text style={styles.subtitle}>
          Versement {paymentIndex}/{totalPayments} — Reçu N° {receipt.receipt_number}
        </Text>

        {/* Meta : numéro, date, méthode */}
        <View style={[styles.metaRow, { marginHorizontal: -4 }]}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.infoBox}>
            ...
            <View style={styles.row}>
              ...
              <Text style={styles.value}>{client.phone_whatsapp}</Text>
            </View>
            {client.email && (
              <View style={styles.row}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.bien}>
            <View style={styles.bienRow}>
              ...
              <Text style={styles.value}>{receipt.property_type === 'terrain' ? 'Terrain' : receipt.property_type === 'maison' ? 'Maison' : 'Autre'}</Text>
            </View>
            {bienLabel && (
              <View style={styles.bienRow}>
                <Text style={styles.label}>Désignation :</Text>
                <Text style={styles.value}>{bienLabel}</Text>
              </View>
            )}
            {lots.length > 0 && (
              <View style={styles.bienRow}>
                <Text style={styles.label}>Lots :</Text>
                <Text style={styles.value}>{lots.map(l => `Îlot ${l.ilot_number} / Lot ${l.lot_number}`).join(' · ')}</Text>
              </View>
            )}
            <View style={[styles.divider, { marginTop: 8 }]} />
            <View style={styles.bienRow}>
              <Text style={styles.label}>Montant total :</Text>
              <Text style={styles.value}>{formatCFA(receipt.total_amount)}</Text>
            </View>
            <View style={styles.bienRow}>
              <Text style={styles.label}>Déjà versé :</Text>
              <Text style={[styles.value, { color: '#059669' }]}>{formatCFA(receipt.amount_paid)}</Text>
            </View>
            <View style={styles.bienRow}>
              <Text style={styles.label}>Reste dû :</Text>
              <Text style={[styles.value, { color: receipt.amount_due > 0 ? '#DC2626' : '#059669' }]}>
                {receipt.amount_due > 0 ? formatCFA(receipt.amount_due) : '✓ Soldé'}
              </Text>
            </View>
          </View>
        </View>

        {/* Montant versé — bloc central */}
        <View style={styles.amountBox}>
          ...
          {payment.amount_words && (
            <Text style={styles.amountWords}>{payment.amount_words}</Text>
          )}
